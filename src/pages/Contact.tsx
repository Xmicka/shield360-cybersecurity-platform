            ...
            <div className="relative z-10" style={{ padding: "128px 32px 96px" }}>
                <div style={{ maxWidth: 1100, margin: "0 auto" }}>
                    <motion.div {...fadeIn} className="text-center mb-16">
                        ...
                        <h1 className="text-4xl sm:text-5xl font-bold text-[#1A1A2E] mb-4 tracking-tight">Get in <span className="gradient-text">touch</span></h1>
                        <p className="text-lg text-[#8A8A8A] max-w-xl mx-auto">Have questions? We'd love to hear from you. Our team typically responds within 24 hours.</p>
                    </motion.div>
                        ...
                        <motion.div {...fadeIn} className="lg:col-span-3 glass p-8">
                            {sent ? (
                                <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center py-12">
                                    ...
                                    <p className="text-sm text-[#8A8A8A]">We'll get back to you within 24 hours.</p>
                                </motion.div>
                            ) : (
                                <form onSubmit={handleSubmit} className="space-y-5">
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        <div><label className="block text-[11px] uppercase tracking-wider text-[#8A8A8A] font-semibold mb-2">Name</label><input type="text" required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="Your name" className="input-premium" /></div>
                                        <div><label className="block text-[11px] uppercase tracking-wider text-[#8A8A8A] font-semibold mb-2">Email</label><input type="email" required value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} placeholder="you@company.com" className="input-premium" /></div>
                                    </div>
                                    <div><label className="block text-[11px] uppercase tracking-wider text-[#8A8A8A] font-semibold mb-2">Subject</label><input type="text" required value={form.subject} onChange={(e) => setForm({ ...form, subject: e.target.value })} placeholder="How can we help?" className="input-premium" /></div>
                                    <div><label className="block text-[11px] uppercase tracking-wider text-[#8A8A8A] font-semibold mb-2">Message</label><textarea required rows={5} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} placeholder="Tell us more..." className="input-premium resize-none" /></div>
                        ...
                        <motion.div {...fadeIn} className="lg:col-span-2 space-y-6">
                            {[
                                ...
                                <div key={info.label} className="glass p-5 flex items-start gap-4">
                                    ...
                                    <div><p className="text-[11px] uppercase tracking-wider text-[#8A8A8A] font-semibold mb-0.5">{info.label}</p><p className="text-sm text-[#1A1A2E]">{info.value}</p></div>
                                </div>
                            ))}
                        </motion.div>
                    </div>
                    ...
                    <motion.div {...fadeIn} className="mt-24 max-w-3xl mx-auto">
                        <h2 className="text-2xl font-bold text-[#1A1A2E] text-center mb-10">Frequently Asked Questions</h2>
    ...
    return (
        <div className="glass overflow-hidden">
            <button onClick={() => setOpen(!open)} className="w-full flex items-center justify-between p-5 text-left">
                <span className="text-sm font-semibold text-[#1A1A2E] pr-4">{q}</span>
            ...
            <motion.div initial={false} animate={{ height: open ? "auto" : 0, opacity: open ? 1 : 0 }} className="overflow-hidden">
                <p className="px-5 pb-5 text-sm text-[#8A8A8A] leading-relaxed">{a}</p>
            </motion.div>
        </div>
    );
}
